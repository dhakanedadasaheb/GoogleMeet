...
const ChatPanel = ({isOpen, onClose,messages, onSendMessage, currentUser}) => {
  ...
  return (
    <aside className=' w-full sm:w-80 h-full bg-white border-l border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200'>
      {/* header  */}
      <div className=' p-4 border-p border-slate-200 flex items-center justify-between'>
        <h3 className=' font-medium text-slate-900 text-base flex items-center gap-2'>In Meeting Chat</h3>
        <button onClick={onClose} className=' p-1.5 rounded-xl hover:bg-slate-100 text-slate-50 hover:text-slate-900 transition-colors cursor-pointer'>
          ...
          messages.map((msg,i)=>{
            ...
            return(
              <div key={msg.id || i} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                ...
                <div className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xs ${isMe ? "bg-primary text-white rounded-tr-none font-medium" : "bg-slate-200 text-slate-800 rounded-tl-none border border-slate-200 font-medium"}`}>{msg.text}</div>
              </div>
            )
          })
        )}
