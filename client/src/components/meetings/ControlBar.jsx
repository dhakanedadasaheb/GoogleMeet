    ...
    <footer className=' w-full bg-white/90 backdrop-blur-md border-t border-slate-200/80 px-6 py-4 flex items-center justify-between z-40 shadow-lg shadow-slate-200/50'>
      {/* left  */}
        <div className=' hidden sm:flex items-center gap-3'>
            <span className=' text-xs font-medium text-slate-600 font-mono tracking-wide'>Id: {roomId}</span>
      ...
      <div className=' flex items-center gap-3 mx-auto sm:mx-0'>
        ...
        title="Toggle In-Meeting Chat">
            ...
            {unreadCount > 0 && !isChatOpen && (
                <span className=' absolute -top-1 -right-1 bg-slate-200 text-slate-800 text-[10px] font-bold px-1.5 py-0.5 rounded-full border border-slate-300'>{participantCount}</span>
            )}
        </button>
        ...
        title="Toggle Participant List">
            <UserIcon className='w-5 h-5'/>
                ...
                <span className=' absolute -top-1 -right-1 bg-primary text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center border-2 border-white shadow-xs'>{participantCount}</span>
        ...
        {isHost ? (
            ...
            className=' p-3.5 rounded-2xl bg-red-600 hover:bg-red-700 text-white shadow-lg shadow-red-500/25 transition-all cursor-pointer border border-red-500 ml-2 font-medium text-xs flex items-center gap-1.5' title='End Meeting for All'>
                <PhoneOffIcon className=' w-5 h-5'/>
                <span className=' hidden md:inline'></span>
            </button>
        ): (
            <button onClick={onLeave}
            className=' p-3.5 rounded-2xl bg-red-600 hover:bg-red-700 text-white shadow-lg shadow-red-500/25 transition-all cursor-pointer border border-red-500 ml-2' title='Leave Meeting'>
                <PhoneOffIcon className=' w-5 h-5'/>
            </button>
        )}
      </div>
